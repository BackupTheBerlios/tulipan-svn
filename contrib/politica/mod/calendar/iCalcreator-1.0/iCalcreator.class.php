...
define( 'ICALCREATOR_VERSION', 'iCalcreator 1.0' );
/*********************************************************************************/
/*********************************************************************************/
/**
 * vcalendar class
 ...
 * set unique_id
 *
 * to bee removed in future releases.. ., use setConfig(.. .
 *
 * @author Kjell-Inge Gustafsson <ical@kigkonsult.se>
 * @since 0.3.0 - 2006-08-10
 ...
 */
  function createXprop() {
    ...
    foreach( $this->xprop as $xpropPart ) {
      ...
      $attributes = $toolbox->_createParams( $params, array( 'LANGUAGE' ));
  ...
  function setConfig( $config, $value ) {
    switch( strtoupper( $config )) {
      ...
      case 'DIRECTORY':
        ...
        break;
      case 'FILENAME':
        if( @touch( $this->getConfig('directory').$this->getConfig('delimiter').$value )) {
          $this->filename = $value;
          return TRUE;
        }
        else
 ...
 *
 * to bee removed in future releases.. ., use getConfig(.. .
 *
 * @author Kjell-Inge Gustafsson <ical@kigkonsult.se>
 ...
 */
  function appendCalendar ( $directory=FALSE, $filename=FALSE, $delimiter=DIRECTORY_SEPARATOR ) {
    if( 'xcal' == $this->format )
      return false;
    if( $directory )
 ...
 *
 * @author Kjell-Inge Gustafsson <ical@kigkonsult.se>
 * @since 0.9.16 - 2007-03-12
 * @param string $directory optional
 * @param string $filename optional
 * @param string $delimiter optional
 * @return bool
 */
  function setFilename( $directory=FALSE, $filename=FALSE, $delimiter=DIRECTORY_SEPARATOR ) {
    if( !empty( $directory ))
 ...
 * @return redirect
 */
  function useCachedCalendar( $directory, $filename, $delimiter=DIRECTORY_SEPARATOR, $timeout=3600) {
    $this->setConfig( 'directory', $directory );
    $this->setConfig( 'filename',  $filename );
    $this->setConfig( 'delimiter', $delimiter );
    $dirfile = $this->getConfig( 'dirfile' );
 ...
 */
  function setAction( $value, $xparam=FALSE ) {
    ...
    $this->action['xparams'] = $this->_setParams( $xparam );
  }
/*********************************************************************************/
/**
 ...
 */
  function createAttendee() {
    ...
    foreach( $this->attendee as $attendeePart ) {                      // start foreach 1
            ...
            foreach( $optparam as $optparamlabel => $optparamvalue ) { // start foreach 4
              ...
              switch( $optparamlabel ) {                               // start switch
                ...
                case 'RSVP':
                  ...
                  break;
                case 'SENT-BY':
                  $attendee1 .= $this->intAttrDelimiter.'SENT-BY='.'"'.$optparamvalue.'"';
                  break;
                case 'MEMBER':
      ...
      if( isset( $attendeeCN )) {
        if( !isset( $attendeeLANG ) && ( '' < $this->getConfig( 'language' )))
          $attendeeLANG = $this->intAttrDelimiter.'LANGUAGE='.$this->getConfig( 'language' );
      }
      else
        $attendeeLANG = null;
      ...
      foreach( $optparam as $optparamlabel => $optparamvalue ) {
        ...
        switch( $optparamlabel ) {
          ...
          default:
            ...
            $attendee['optparam'][] = array( $optparamlabel => $optparamvalue );
            break;
        } // end switch( $optparamlabel.. .
      } // end foreach( $optparam.. .
      foreach( $optarrays as $optparamlabel => $optparams )
        $attendee['optparam'][] = array( $optparamlabel => $optparams );
    }
 ...
 */
  function createCategories() {
    ...
    $content    = implode( ',', $this->categories['value'] );
    if( 0 < ( substr_count($content, ', ') + substr_count($content, ' ,'))) {
 ...
 */
  function setCategories( $value, $params=FALSE ) {
    ...
    $this->categories['param'] = $this->_setParams( $params );
  }
/*********************************************************************************/
/**
 * Property Name: CLASS
 ...
 */
  function setClass( $value, $xparams=FALSE ) {
    ...
    $this->class['xparams'] = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 ...
 */
  function setCompleted( $year, $month=FALSE, $day=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $xparam=FALSE ) {
    if( is_array( $year ) &&
      ...
      $this->completed['xparams'] = $this->_setParams( $month );
    }
    elseif( is_array( $year ) && isset( $year['timestamp'] )) {
    ...
    elseif( 8 <= strlen( trim( $year ))) { // ex. 2006-08-03 10:12:18
      ...
      $this->completed['value'] = array('year'  => $year
                                      ...
                                      , 'min'   => $min
                                      , 'sec'   => $sec);
      $this->completed['xparams'] = $this->_setParams( $xparam );
    }
 ...
 */
  function setContact( $value, $params=FALSE ) {
    ...
    $this->contact['params'] = $this->_setParams( $params );
  }
/*********************************************************************************/
/**
 ...
 */
  function setCreated( $year, $month=FALSE, $day=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $xparams=FALSE ) {
    if( is_array( $year ) &&
      ...
      $this->created['xparams'] = $this->_setParams( $month );
    }
    elseif( is_array( $year ) && isset( $year['timestamp'] )) {
    ...
    elseif( 8 <= strlen( trim( $year ))) { // ex. 2006-08-03 10:12:18
      ...
      $this->created['value'] = array( 'year'  => $year
                                     ...
                                     , 'min'   => $min
                                     , 'sec'   => $sec );
      $this->created['xparams'] = $this->_setParams( $xparams );
    }
 ...
 */
  function createDtend() {
    ...
    return $this->_createElement( 'DTEND', $attributes, $formatted[0] );
  }
/**
 ...
 */
  function setDtend( $year, $month=FALSE, $day=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $tz=FALSE, $xparams=FALSE ) {
    ...
    if( is_array( $year ) && ( in_array( count( $year ), array( 3, 4, 6, 7 )))) {
      $this->dtend['xparams'] = $this->_setParams( $month );
      $hitval = ( !empty( $year['tz'] )) ? 7 : 6;
      $parno = $this->_existRem( $this->dtend['xparams'], 'VALUE', 'DATE-TIME', $hitval );
      if( !isset( $parno )) $parno = $this->_existRem( $this->dtend['xparams'], 'VALUE', 'DATE', 3 );
    ...
    elseif( 8 <= strlen( trim( $year ))) { // ex. 2006-08-03 10:12:18
      $this->dtend['xparams'] = $this->_setParams( $month );
      $parno = $this->_existRem( $this->dtend['xparams'], 'VALUE', 'DATE-TIME', 7 );
      if( !isset( $parno )) $parno = $this->_existRem( $this->dtend['xparams'], 'VALUE', 'DATE', 3 );
      $this->dtend['value'] = $this->_date_time_string( $year, $parno );
    }
    else {
      $this->dtend['xparams'] = $this->_setParams( $xparams );
 ...
 */
  function setDtstamp( $year, $month=FALSE, $day=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $xparams=FALSE ) {
    if( is_array( $year ) &&
      ...
      $this->dtstamp['xparams'] = $this->_setParams( $month );
    }
    elseif( is_array( $year ) && isset( $year['timestamp'] )) {
    ...
    elseif( 8 <= strlen( trim( $year ))) { // ex. 2006-08-03 10:12:18
      ...
      $this->dtstamp['value'] = array( 'year'  => $year
                                     ...
                                     , 'min'   => $min
                                     , 'sec'   => $sec );
      $this->dtstamp['xparams'] = $this->_setParams( $xparams );
    }
 ...
 */
  function createDtstart( $localtime=FALSE ) {
    ...
    return $this->_createElement( 'DTSTART', $attributes, $formatted[0] );
  }
/**
 ...
 */
  function setDtstart( $year, $month=FALSE, $day=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $tz=FALSE, $xparams=FALSE ) {
    $parno = null;
    if( is_array( $year ) && ( in_array( count( $year ), array( 3, 4, 6, 7 )))) {
      $this->dtstart['xparams'] = $this->_setParams( $month );
      $hitval = ( !empty( $year['tz'] )) ? 7 : 6;
      $parno = $this->_existRem( $this->dtstart['xparams'], 'VALUE', 'DATE-TIME', $hitval );
      if( !isset( $parno )) $parno = $this->_existRem( $this->dtstart['xparams'], 'VALUE', 'DATE', 3 );
      if( !isset( $parno )) $parno = count( $year );
    ...
    elseif( 8 <= strlen( trim( $year ))) { // ex. 2006-08-03 10:12:18
      $this->dtstart['xparams'] = $this->_setParams( $month );
      $parno = $this->_existRem( $this->dtstart['xparams'], 'VALUE', 'DATE-TIME', 7 );
      if( !isset( $parno )) $parno = $this->_existRem( $this->dtstart['xparams'], 'VALUE', 'DATE', 3 );
      $this->dtstart['value'] = $this->_date_time_string( $year, $parno );
    }
    else {
      $this->dtstart['xparams'] = $this->_setParams( $xparams );
      $hitval = ( !empty( $tz )) ? 7 : null;
      $parno = $this->_existRem( $this->dtstart['xparams'], 'VALUE', 'DATE-TIME', $hitval );
      if( !isset( $parno )) $parno = $this->_existRem( $this->dtstart['xparams'], 'VALUE', 'DATE', 3 );
 ...
 */
  function createDue() {
    ...
    return $this->_createElement( 'DUE', $attributes, $formatted[0] );
  }
/**
 ...
 */
  function setDue( $year, $month=FALSE, $day=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $tz=FALSE, $xparams=FALSE ) {
    $parno = null;
    if( is_array( $year ) && ( in_array( count( $year ), array( 3, 4, 6, 7 )))) {
      $this->due['xparams'] = $this->_setParams( $month );
      $hitval = ( !empty( $year['tz'] )) ? 7 : 6;
      $parno = $this->_existRem( $this->due['xparams'], 'VALUE', 'DATE-TIME', $hitval );
      if( !isset( $parno )) $parno = $this->_existRem( $this->due['xparams'], 'VALUE', 'DATE', 3 );
      if( !isset( $parno )) $parno = count( $year );
    ...
    elseif( 8 <= strlen( trim( $year ))) { // ex. 2006-08-03 10:12:18
      $this->due['xparams'] = $this->_setParams( $month );
      $parno = $this->_existRem( $this->due['xparams'], 'VALUE', 'DATE-TIME', 7 );
      if( !isset( $parno )) $parno = $this->_existRem( $this->due['xparams'], 'VALUE', 'DATE', 3 );
      $this->due['value'] = $this->_date_time_string( $year, $parno );
    }
    else {
      $this->due['xparams'] = $this->_setParams( $xparams );
      $hitval = ( !empty( $tz )) ? 7 : null;
      $parno = $this->_existRem( $this->due['xparams'], 'VALUE', 'DATE-TIME', $hitval );
      if( !isset( $parno )) $parno = $this->_existRem( $this->due['xparams'], 'VALUE', 'DATE', 3 );
 ...
 */
  function setDuration( $week=FALSE, $day=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $xparams=FALSE ) {
    if( is_array( $week ))  {
      ...
      $this->duration['xparams'] = $this->_setParams( $day );
    }
    elseif( is_string( $week ) && ( 3 <= strlen( trim( $week )))) {
      ...
      $this->duration['xparams'] = $this->_setParams( $xparams );
    }
  }
/*********************************************************************************/
/**
 ...
 */
  function setExdate( $exdates, $params=FALSE ) {
    $exdate = array();
    $exdate['params'] = $this->_setParams( $params );
    $parno  = $this->_existRem( $exdate['params'], 'VALUE', 'DATE-TIME', 7 );
 ...
 */
  function setFreebusy( $fbType, $fbValues, $xparams=FALSE ) {
    ...
    $freebusy['xparams'] = $this->_setParams( $xparams );
 ...
 */
  function setGeo( $latitude, $longitude, $xparams=FALSE ) {
    ...
    $this->geo['xparams']   = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 ...
 */
  function setLastModified( $year, $month=FALSE, $day=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $xparams=FALSE ) {
    if( is_array( $year ) &&
      ...
      $this->lastmodified['xparams'] = $this->_setParams( $month );
    }
    elseif( is_array( $year ) && isset( $year['timestamp'] )) {
      $this->lastmodified['value']   = $this->_date_time_string( date( 'Y-m-d H:i:s', $year['timestamp'] ), 7 );
      $this->lastmodified['xparams'] = $this->_setParams( $month );
    }
    elseif( 8 <= strlen( trim( $year ))) { // ex. 2006-08-03 10:12:18
      ...
      $this->lastmodified['value']   = array( 'year'  => $year
                                            ...
                                            , 'sec'   => $sec
                                            , 'tz'    => 'Z');
      $this->lastmodified['xparams'] = $this->_setParams( $xparams );
    }
    if( isset( $this->lastmodified['value']['year'] ) &&
  ...
  function setOrganizer( $value, $params=FALSE ) {
    ...
    if( isset($this->organizer['params']['SENT-BY'] )) {
      $this->organizer['params']['SENT-BY'] =
        str_replace( 'MAILTO:', '', $this->organizer['params']['SENT-BY'] );
      $this->organizer['params']['SENT-BY'] =
        str_replace( 'mailto:', '', $this->organizer['params']['SENT-BY'] );
    }
  }
 ...
 */
  function setPercentComplete( $value, $xparams=FALSE ) {
    ...
    $this->percentcomplete['xparams'] = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 ...
 */
  function setPriority( $value, $xparams=FALSE  ) {
    ...
    $this->priority['xparams'] = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 * Property Name: RDATE
 ...
 */
  function setRdate( $rdates, $xparams=FALSE ) {
    $input   = array();
    $input['xparams'] = $this->_setParams( $xparams );
    $parno   = $this->_existRem( $input['xparams'], 'VALUE', 'PERIOD' );
    $parno   = $this->_existRem( $input['xparams'], 'VALUE', 'DATE-TIME', 7 );
    if( !isset( $parno )) $parno = $this->_existRem( $input['xparams'], 'VALUE', 'DATE', 3 );
 ...
 */
  function createRecurrenceid() {
    ...
    $attributes  = ( isset( $formatted[2] )) ? $formatted[2] : $formatted[1];
    $attributes .= ( isset( $this->recurrenceid['xparams'] )) ? $this->_createParams( $this->recurrenceid['xparams'] ) : null;
 ...
 * @return void
 */
  function setRecurrenceid( $year, $month=FALSE, $day=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $tz=FALSE, $xparams=FALSE ) {
    if( is_array( $year ) && ( in_array( count( $year ), array( 3, 4, 6, 7 )))) {
      $this->recurrenceid['xparams'] = $this->_setParams( $month );
      $hitval = ( !empty( $year['tz'] )) ? 7 : 6;
      $parno = $this->_existRem( $this->recurrenceid['xparams'], 'VALUE', 'DATE-TIME', $hitval );
      if( !isset( $parno )) $parno = $this->_existRem( $this->recurrenceid['xparams'], 'VALUE', 'DATE', 3 );
      if( !isset( $parno )) $parno = count( $year );
    ...
    elseif( 8 <= strlen( trim( $year ))) { // ex. 2006-08-03 10:12:18
      $this->recurrenceid['xparams'] = $this->_setParams( $month );
      $parno = $this->_existRem( $this->recurrenceid['xparams'], 'VALUE', 'DATE-TIME', 7 );
      if( !isset( $parno )) $parno = $this->_existRem( $this->recurrenceid['xparams'], 'VALUE', 'DATE', 3 );
      $this->recurrenceid['value'] = $this->_date_time_string( $year, $parno );
    }
    else {
      $this->recurrenceid['xparams'] = $this->_setParams( $xparams );
      $hitval = ( !empty( $tz )) ? 7 : null;
      $parno = $this->_existRem( $this->recurrenceid['xparams'], 'VALUE', 'DATE-TIME', $hitval );
      if( !isset( $parno )) $parno = $this->_existRem( $this->recurrenceid['xparams'], 'VALUE', 'DATE', 3 );
      $this->recurrenceid['value'] = array( 'year'  => $year, 'month' => $month, 'day'   => $day );
      if (( $hour || $min || $sec ) && ( 3 != $parno )) {
        $this->recurrenceid['value']['hour'] = $hour;
        $this->recurrenceid['value']['min']  = $min;
        $this->recurrenceid['value']['sec']  = $sec;
        if( !empty( $tz ))
 ...
 */
  function setRepeat( $value, $xparams=FALSE ) {
    ...
    $this->repeat['xparams'] = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 ...
 */
  function setRequestStatus( $statcode, $text, $extdata=FALSE, $params=FALSE ) {
    ...
    $input['param'] = $this->_setParams( $params);
 ...
 */
  function setRrule( $rruleset, $xparams=FALSE ) {
    ...
    $rrule['xparams'] = $this->_setParams( $xparams );
    $this->rrule[]    = $rrule;
  }
/*********************************************************************************/
/**
 ...
 */
  function setSequence( $value, $xparams=FALSE ) {
    ...
    $this->sequence['xparams'] = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 ...
 */
  function setStatus( $value, $xparams=FALSE ) {
    ...
    $this->status['xparams'] = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 ...
 */
  function setTransp( $value, $xparams=FALSE ) {
    ...
    $this->transp['xparams'] = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 ...
 */
  function setTrigger( $year=FALSE, $month=FALSE, $day=FALSE, $week=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $relatedEnd=FALSE, $after=FALSE, $tz=FALSE, $xparams=FALSE ) {
    if( is_array( $year ) && array_key_exists( 'timestamp', $year )) { // timestamp
      $xparams = $this->_setParams( $month );
      $this->_existRem( $xparams, 'VALUE', 'DATE-TIME' );
      $tz    = ( isset( $year['tz'] )) ? $year['tz'] : null;
      $parno = ( isset( $tz )) ? 7 : 6;
      $date  = $this->_date_time_string( date( 'Y-m-d H:i:s', $year['timestamp'] ), $parno );
      foreach( $date as $k => $v )
        $$k = $v;
    }
    elseif( is_array( $year )) {
      if( array_key_exists( 'year',  $year ) &&
        ...
        $relatedEnd = $month;
        $after      = $day;
        $xparams = $this->_setParams( $week );
      }
      $this->_existRem( $xparams, 'VALUE', 'DATE-TIME' );
      $this->_existRem( $xparams, 'VALUE', 'DURATION' );
    ...
    elseif( is_string($year) && !is_int( $year )) {  // duration or date in a string
      ...
      $xparams = $this->_setParams( $xparams );
    ...
    //  if( !empty( $sec ))
      ...
      $this->_existRem( $xparams, 'VALUE', 'DURATION' );
      if( $this->_existRem( $xparams, 'RELATED', 'START', TRUE ))
        $relatedEnd = FALSE;
      elseif( $this->_existRem( $xparams, 'RELATED', 'END', TRUE ))
        $relatedEnd = TRUE;
      if( !empty( $week )) {
 ...
 */
  function setTzid( $value, $xparams=FALSE ) {
    ...
    if( !isset( $this->tzid['value'] ))
      return;
    $this->tzid['xparams'] = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 * .. .
 ...
 */
  function setTzname( $value, $params=FALSE ) {
    ...
    if( empty( $this->tzname['value'] ))
      return;
    $this->tzname['params'] = $this->_setParams( $params );
  }
/*********************************************************************************/
/**
 ...
 */
  function setTzoffsetfrom( $value, $xparams=FALSE ) {
    ...
    if( empty( $this->tzoffsetfrom['value'] ))
      return;
    $this->txoffsetfrom['xparams'] = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 ...
 */
  function setTzoffsetto( $value, $xparams=FALSE ) {
    ...
    if( empty( $this->tzoffsetto['value'] ))
      return;
    $this->tzoffsetto['xparams'] = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 * Property Name: TZURL
 ...
 */
  function setTzurl( $value, $xparams=FALSE ) {
    ...
    if( empty( $this->tzurl['value'] ))
      return;
    $this->tzurl['xparams'] = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 * Property Name: UID
 ...
 */
  function setUid( $value, $xparams=FALSE ) {
    ...
    if( empty( $this->uid['value'] ))
      return;
    $this->uid['xparams'] = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 * Property Name: URL
 ...
 */
  function setUrl( $value, $xparams=FALSE ) {
    ...
    if( empty( $this->url['value'] ))
      return;
    $this->url['xparams'] = $this->_setParams( $xparams );
  }
/*********************************************************************************/
/**
 * Property Name: x-prop
...
/*********************************************************************************/
/**
 * get general component config variables
 *
 * @author Kjell-Inge Gustafsson <ical@kigkonsult.se>
 * @since 0.9.22 - 2007-04-06
 * @param string $config
